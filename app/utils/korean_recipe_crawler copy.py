...
async def get_recipe_by_id(recipe_id: int):
    """특정 레시피 ID로 상세 정보를 가져옵니다."""
    try:
        # recipe_id에서 실제 ID 추출 (10000recipe_12345 -> 12345)
        if isinstance(recipe_id, str) and recipe_id.startswith("10000recipe_"):
            actual_id = recipe_id.replace("10000recipe_", "")
        else:
            actual_id = str(recipe_id)
        
        # # logger.info(f"만개의레시피에서 레시피 상세 정보 조회: ID={actual_id}")
        
        # 크롤러 인스턴스 생성
        crawler = KoreanRecipeCrawler()
        
        # 상세 정보 가져오기
        async with aiohttp.ClientSession(headers=crawler.headers, timeout=aiohttp.ClientTimeout(total=crawler.timeout)) as session:
            recipe_detail = await crawler._get_recipe_detail(session, actual_id)
            
            if recipe_detail:
                # # logger.info(f"만개의레시피 레시피 상세 정보 조회 성공: ID={actual_id}")
                return recipe_detail
            else:
                logger.warning(f"만개의레시피에서 레시피를 찾을 수 없습니다: ID={actual_id}")
                return None
                
    except Exception as e:
        logger.error(f"만개의레시피 레시피 상세 정보 조회 중 오류: {e}")
        return None
